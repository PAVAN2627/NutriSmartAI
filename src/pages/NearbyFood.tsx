...
function mapsLink(name: string, id: string) {
...
export default function NearbyFood() {
  ...
  const [mapsReady, setMapsReady] = useState(!!window.google?.maps?.places?.Place);
  const [search, setSearch] = useState("");
  const [activeFilter, setActiveFilter] = useState("All");
  const mapDivRef = useRef<HTMLDivElement>(null);
  const filters = ["All", "Open Now", "Top Rated", "Nearest"];

  // Load Maps JS SDK with async pattern
  useEffect(() => {
  ...
  const searchNearby = async (lat: number, lng: number) => {
    ...
    setLoading(true);
    try {
      // Use legacy PlacesService (still works, just not for new billing accounts)
      // Fallback: use text search via Places API new
      const map = new window.google.maps.Map(mapDivRef.current, { center: { lat, lng }, zoom: 14 });
      const service = new window.google.maps.places.PlacesService(map);

      service.nearbySearch(
        { location: { lat, lng }, radius: 3000, type: "restaurant", keyword: "healthy food" },
        (results: any[], status: string) => {
          setLoading(false);
          const ok = window.google.maps.places.PlacesServiceStatus.OK;
          const zero = window.google.maps.places.PlacesServiceStatus.ZERO_RESULTS;
          if (status === ok && results?.length) {
            const items: PlaceItem[] = results.map((p: any) => ({
              id: p.place_id,
              name: p.name,
              address: p.vicinity || "",
              rating: p.rating,
              ...
              photoUrl: p.photos?.[0]?.getUrl({ maxWidth: 160, maxHeight: 160 }),
            ...
            setPlaces(items);
            toast.success(`Found ${items.length} places near you!`);
          } else if (status === zero) {
            setPlaces([]);
            toast.info("No healthy restaurants found nearby.");
          } else {
            toast.error("Places search failed. Make sure Places API is enabled in Google Cloud Console.");
          }
        }
      );
    } catch (err) {
  ...
  const getLocation = () => {
    ...
    if (!mapsReady) return toast.error("Maps still loading, try again in a moment");
    setLoading(true);
  ...
  useEffect(() => { if (mapsReady) getLocation(); }, [mapsReady]);

  let filtered = places.filter((p) => !search || p.name.toLowerCase().includes(search.toLowerCase()));
  ...
  if (activeFilter === "Nearest") filtered = [...filtered].sort((a, b) => parseFloat(a.distance) - parseFloat(b.distance));
    ...
    <DashboardLayout>
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-muted-foreground">
                {location ? `${location.lat.toFixed(4)}, ${location.lng.toFixed(4)} · 3km radius` : "Allow location access to find nearby places"}
              </p>
            </div>
        ...
        {!loading && !location && (
          <Card className="shadow-card border-dashed border-primary/30">
            <CardContent className="p-12 text-center">
              ...
              <p className="text-muted-foreground text-sm mb-6">We need your location to find healthy restaurants nearby.</p>
        ...
        {!loading && filtered.length > 0 && (
          <div className="space-y-4">
            ...
            {filtered.map((place, i) => (
              <Card key={place.id} className="shadow-card hover:shadow-elevated transition-all hover:-translate-y-0.5 animate-fade-in" style={{ animationDelay: `${i * 0.05}s` }}>
                <CardContent className="p-4 flex gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <a href={mapsLink(place.name, place.id)} target="_blank" rel="noopener noreferrer" className="shrink-0 text-muted-foreground hover:text-primary transition-colors">
                        <ExternalLink className="h-4 w-4" />
                      </a>
                    </div>
                    <p className="text-xs text-muted-foreground mt-0.5 truncate">{place.address}</p>
                    <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1"><MapPin className="h-3 w-3" />{place.distance} km</span>
                      {place.rating && (
                        <span className="flex items-center gap-1 text-nutri-orange">
                          <Star className="h-3 w-3 fill-nutri-orange" />{place.rating} ({place.userRatingCount})
                        </span>
                      )}
                      {place.isOpen !== undefined && (
                        <span className={`flex items-center gap-1 font-medium ${place.isOpen ? "text-primary" : "text-destructive"}`}>
                          <Clock className="h-3 w-3" />{place.isOpen ? "Open Now" : "Closed"}
                        </span>
                      )}
                    </div>
                  </div>
        ...
        {!loading && location && places.length === 0 && (
          <Card className="shadow-card border-nutri-orange/20 bg-nutri-orange/5">
            <CardContent className="p-4 text-sm text-muted-foreground">
              No results? Make sure <span className="font-semibold text-foreground">Places API</span> and <span className="font-semibold text-foreground">Maps JavaScript API</span> are both enabled in your Google Cloud Console for this API key.
            </CardContent>
          </Card>
        )}
      </div>
